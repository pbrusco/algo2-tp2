...
\Orden{O(version(d))}
\BlankLine
\eIf{version(d) $\notin$ e.versiones}{direccionNoSoportada()}{
\textbf{var} it: itOrd(secuOrdSinRep(evento)) \\
it $\leftarrow$ crearIt(e.reglas)\\
\While{actual(it).version $\neq$ version(d)}{avanzar(it)}
\eIf{$\neq$ tieneRegla(actual(it).aR,d)}{interfazDeSalidaNoEncontrada()}{
\eIf{estaCaida?(e,interfazDeSalida(actual(it).aR,d))}{interfazDeSalidaCaida(interfazDeSalida(actual(it).aR,d))}{salidaPorInterfaz(interfazDeSalida(actual(it).aR,d))}
}}
\end{algorithm}
