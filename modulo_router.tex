...
\Else{\If{(tieneProximo(it)?)\tcp*{O(1)}}{\While{(actual(it) $\neq$ m)\tcp*{O(a)}}
	...
	\BlankLine
	\If{($\neg$tieneProximo?(it))\tcp*{O(1)}}{\If{(m.estaCaida?)\tcp*{O(1)}}{tc + (estado$\_$post.timestamp - m.timestamp)\tcp*{O(1)}}}
	\Else{avanzar(it)\tcp*{O(1)} 
		estado$\_$prev $\leftarrow$ actual(it)\tcp*{O(1)}
		\BlankLine 
		\If{(m.estaCaida? $\wedge\ \neg$estado$\_$prev.estaCaida?)\tcp*{O(1)}}{tc + (estado$\_$post.timestamp - m.timestamp)\tcp*{O(1)}}
		\BlankLine 
		\If{($\neg$m.estaCaida? $\wedge$ estado$\_$prev.estaCaida?)\tcp*{O(1)}}{tc - (estado$\_$post.timestamp - m.timestamp)\tcp*{O(1)}}}}}
\end{algorithm}
