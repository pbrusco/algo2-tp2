...
insertarOrdenadoDesdeAdelante($\langle$v, $<>\rangle$, e.reglas)\tcp*{O(v)}
\end{algorithm}
...
{avanzar(it)}
agRegla(actualAdelante(it).aR, u) \tcp*{O(cantBits(u))}
\end{algorithm}
...
\BlankLine
insertar(m, (e.status$\_$inter[i]).eventos) \tcp*{O(a)}\footnote{a = longitud(e.status$\_$inter[i] - PosicionRelativa(e.status$\_$inter[i],m)}
\BlankLine
it $\leftarrow$ crearIt((e.status$\_$inter[i]).eventos) \tcp*{O(1)}
\BlankLine
\If{(actualAdelante(it) $=$ m) $\wedge$ tieneProximo?(it))\tcp*{O(1)}}
	{avanzar(it) \tcp*{O(1)}
	 \If{(actualAdelante(it).estaCaida?)\tcp*{O(1)}}{tc $\leftarrow$ tc + (m.timestamp - actualAdelante(it).timestamp)\tcp*{O(1)}}}
\Else{\If{(tieneProximo(it)?)\tcp*{O(1)}}{\While{(actualAdelante(it) $\neq$ m)\tcp*{O(a)}}
		{estado$\_$post $\leftarrow$ actualAdelante(it) \tcp*{O(1)} avanzar(it)\tcp*{O(1)}}
	\BlankLine
	...
	\Else{avanzar(it)\tcp*{O(1)} 
		estado$\_$prev $\leftarrow$ actualAdelante(it)\tcp*{O(1)}
		\BlankLine 
...
\eIf{($\neg$ tieneRegla(actualAdelante(it).aR,d)) \tcp*{O(version(d))}}{res $\leftarrow$ interfazDeSalidaNoEncontrada()\tcp*{O(1)}}{
i $\leftarrow$ interfazDeSalida(actualAdelante(it).aR,d) \tcp*{O(version(d))}
