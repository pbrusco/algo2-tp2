...
\textbf{se explica con la especificaci\'on de} \textit{Router} y \textir{ArbolDeReglas}\\
...
\axioma{interfazDeEventosCorrecta(n,a)}{($\forall$ i:interfaz) (i $<$ n \impluego\ 		sonEventosDeLaInterfaz(a[i].eventos,i))}
...
\axioma{tiempoCaidaValido(a)}{($\forall$ i:interfaz) (i $<$ n \impluego\ a[i].tiempoCaida $=$ sumarCaidas(a[i].eventos)}
...
($\forall$ i: interfaz)(i $<$ interfaces(r) \impluego reverso(eventos(r,i)) \igobs (e.status$\_$inter[i]).eventos) $\wedge$ \\
...
{avanzar(it)}
agRegla(actual(it).aR, u) \tcp*{O(cantBits(u))}
\end{algorithm}
...
\BlankLine
insertar(m, (e.status$\_$inter[i]).eventos) \tcp*{O(posicionRelativa(m))}
\BlankLine
...
i $\leftarrow$ interfazDeSalida(actual(it).aR,d) \tcp*{O(version(d))}
