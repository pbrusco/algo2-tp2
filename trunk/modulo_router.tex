...
\BlankLine
insertarOrdenadoDesdeAtras(m, (e.status$\_$inter[i]).eventos) \tcp*{O(a)\footnote{a = longitud(e.status$\_$inter[i] - PosicionRelativa(e.status$\_$inter[i],m)}}
\BlankLine
...
\Else{\If{(tieneAnterior(it)?)\tcp*{O(1)}}{\While{(actualAtras(it) $\neq$ m)\tcp*{O(a)}}
		{estado$\_$post $\leftarrow$ actualAtras(it) \tcp*{O(1)} retroceder(it)\tcp*{O(1)}}
	\BlankLine
	\If{($\neg$tieneAnterior?(it))\tcp*{O(1)}}{\If{(m.estaCaida?)\tcp*{O(1)}}{tc $\leftarrow$ tc + (estado$\_$post.timestamp - m.timestamp)\tcp*{O(1)}}}
	\Else{retroceder(it)\tcp*{O(1)} 
		estado$\_$prev $\leftarrow$ actualAtras(it)\tcp*{O(1)}
		\BlankLine 
