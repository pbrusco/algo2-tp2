...
\BlankLine
\eIf{(actual(it) $=$ m) $\wedge$ tieneProximo?(it)}
	{avanzar(it)\\
	 \If{(actual(it).estaCaida?)}{tc $\leftarrow$ tc + (m.timestamp - actual(it).timestamp)}
	}{\While{(actual(it) $\neq$ m)}
		{estado$\_$post $\leftarrow$ actual(it)\\ avanzar(it)}
	\BlankLine
	\eIf{($\neg$tieneProximo?(it) $\wedge$ m.estaCaida?)}
		{tc + (estado$\_$post.timestamp - m.timestamp)}{
		avanzar(it)\\ 
		estado$\_$prev $\leftarrow$ actual(it)\\ 
		\If{(m.estaCaida? $\wedge\ \neg$estado$\_$prev.estaCaida?)}{tc + (estado$\_$post.timestamp - m.timestamp)}
		\BlankLine 
		\If{($\neg$m.estaCaida? $\wedge$ estado$\_$prev.estaCaida?)}{tc - (estado$\_$post.timestamp - m.timestamp)}}}
