...
\func{versionesDeReglasValidas}{secuOrdSinRep(tupla(version,abr)),conj(version)}{bool}{}
...
\BlankLine
insertar(m, (e.status$\_$inter[i]).eventos) \tcp*{O(a)}
\BlankLine
...
\eIf{(estaCaida?(e, interfazDeSalida(actual(it).aR, d)))\tcp*{O(version(d))}}{interfazDeSalidaCaida(interfazDeSalida(actual(it).aR, d))\tcp*{O(version(d))}}{res $\leftarrow$ salidaPorInterfaz(interfazDeSalida(actual(it).aR, d))\tcp*{O(version(d))}}
}}
\end{algorithm}
...
it $\leftarrow$ crearIt((e.status$\_$inter[i]).eventos) \tcp*{O(1)}
\BlankLine
\eIf{(it == NULL)\tcp*{O(1)}}{res $\leftarrow$ false\tcp*{O(1)}}{res $\rightarrow$ actual(it).estaCaida? \tcp*{O(1)}}
\end{algorithm}
