...
\restylealgo{boxed}
\dontprintsemicolon
\textit{i}nuevo(in n: nat) $\longrightarrow$ res: estr \tcp*{O(n)} 
...
\textit{i}agVersion(inout e: estr, in v: version)\tcp*{O(v)}
\begin{algorithm}[H]
\BlankLine
...
\textit{i}agRegla(inout e: estr, in u: reglaDir)\tcp*{O(version(dirIp(u))))}
\begin{algorithm}[H]
\BlankLine
...
\textit{i}agEvento(inout e: estr, in m: evento) \tcp*{O(a)}
\begin{algorithm}[H]
FALTA
\end{algorithm}
...
\textit{i}versiones(in e: estr) $\longrightarrow$ res: conj(version)\tcp*{O(1)}
\begin{algorithm}[H]
\BlankLine
...
\textit{i}interfaces(in e: estr) $\longrightarrow$ res: nat\tcp*{O(1)}
\begin{algorithm}[H]
\BlankLine
...
\textit{i}enrutar(inout e: estr, in d: dirIp)\tcp*{O(version(d))}
\begin{algorithm}[H]
\BlankLine
ME FALTA HACERLO
\end{algorithm}
...
\textit{i}eventos(in e: estr, in i: interfaz) $\longrightarrow$ res: secuOrdSinRep(evento)\tcp*{O(1)}
\begin{algorithm}[H]
\BlankLine
...
\textit{i}estaCaida?(in e: estr, in i: interfaz) $\longrightarrow$ res: bool\tcp*{O(1)}
\begin{algorithm}[H]
\BlankLine
...
\textit{i}tiempoCaida(in e: estr, in i: interfaz) $\longrightarrow$ res: nat \tcp*{O(1)}
\begin{algorithm}[H]
