...
\begin{algorithm}[H]
\Orden{O(a)\footnote{a = longitud(e.status$\_$inter[i] - posicionRelativa(e.status$\_$inter[i],m)}}
\BlankLine
...
\Else{\If{(tieneAnterior(it)?)\tcp*{O(1)}}{\While{(actualAtras(it) $\neq$ m)\tcp*{O(a)\footnote{El ciclo se ejecuta a lo sumo ``a'' veces, dado que la secuencia se recorre de atras hacia adelante. Es decir, que el costo temporal del ciclo es igual a la cantidad de eventos en la secuencia con interfaz mayor que la del evento que quiero agregar}}}
...
\Orden{O(version(d))}
\BlankLine
\eIf{($\neg$ pertenece(version(d),e.versiones)) \tcp*{O(1)}}{res $\leftarrow$ direccionNoSoportada() \tcp*{O(posicionRelativa(version(v),e.versiones))\footnote{Cumple esta complejidad por la informac\'ion que aparece en la interfaz Conjunto}} }{
\textbf{var} it: itOrd(secuOrdSinRep(tupla$\langle$version, abr$\rangle$))\\ 
\textbf{var} i: interfaz\\ 
it $\leftarrow$ crearIt(e.reglas) \tcp*{O(1)}
