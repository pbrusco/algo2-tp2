\section*{Modulo \nombretad{Conjunto($\alpha$)}}

%INTERFAZ
\subsection*{Interfaz}
\textbf{interfaz} \textit{Conjunto($\alpha$)}\\
\textbf{se explica con la especificaci\'on de} \textit{Conjunto($\alpha$)}\\
\textbf{usa interfaces} \nombretad{Bool}, \nombretad{Nat}\\
\textbf{genero} \textit{conj($\alpha$)}\\

\textbf{operaciones}\\

$Vacio$()$\longrightarrow res$: conj($\alpha$) \hfill (O(1)) \\
$\{$true$\}$\\
$\{\sombreritoWide{res} \igobs \phi()\}$\\

$agregar$(in $a$: $\alpha$, inout $C$: conj($\alpha$))\hfill (O(posicionRelativa(C,a))) \\
$\{\sombreritoWide{C}\ \igobs\ C_0\}$\\
$\{\sombreritoWide{C}\ \igobs$ Ag($\sombreritoWide{a}$,$C_0$)$\}$\\

$pertenece$(in $a$: $\alpha$, in $C$: conj($\alpha$))$\longrightarrow res$: bool \hfill (O(posicionRelativa(C,a))) \\
$\{$true$\}$\\
$\{\sombreritoWide{res}\ \igobs \sombreritoWide{a}\in \sombreritoWide{C}\}$\\

%REPRESENTACION
\subsection*{Representaci\'on}
\textit{conjunto($\alpha$)} \textbf{se representa con} \textit{estrConj}\\
\textbf{donde} \textit{estrConj} \textbf{es} \textit{SecuOrdSinRep($\alpha$)}\\

%INVARIANTE DE REPRESENTACION
\alinearfuncs{Abs}{\hspace{2cm}}
\subsection*{Invariante\ de \ Representaci\'on}
\alinearfuncs{Rep}{}
\func{Rep}{estrConj}{boolean}{}
\vspace{11pt}
($\forall$ e: estrConj) Rep(e) = true
\vspace{33pt}

%FUNCION DE ABSTRACCION
\subsection*{Funci\'on de Abstracci\'on}
\alinearfuncs{Abs}{}
\func{Abs}{estrConj/e}{conj($\alpha$)}{Rep(e)}
\vspace{11pt}
($\forall$ e: estrConj) Abs(e) = C: conj($\alpha$)/($\forall a:\alpha$)($a\in C\igobs esta?(a,\sombreritoWide{e}$))
\vspace{33pt}


%ALGORITMOS
\subsection*{Algoritmos}

$\textit{i}Vacio$()$\longrightarrow res$: SecuOrdSinRep($\alpha$)\\
\begin{algorithm}[H]
\Orden{O(1)}
\BlankLine
$res\leftarrow <>$
\end{algorithm}

$\textit{i}agregar$(in $a$: $\alpha$, inout $s$: SecuOrdSinRep($\alpha$))\\
\begin{algorithm}[H]
\Orden{O(posicionRelativa(s,a))}
\BlankLine
\If{$\neg$ esta?(a,s)\tcp*{O(posicionRelativa(s,a))\footnote{Esta complejidad se cumple por la informaci\'on que brinda la interfaz de SecuOrdSinRep}}}{insertarOrdenadoDesdeAdelante(a,s)\tcp*{O(posicionRelativa(s,a))\footnote{Esta complejidad se cumple por la informaci\'on que brinda la interfaz de SecuOrdSinRep}}} 

\end{algorithm}

$ipertenece$(in $a$: $\alpha$, in $s$: SecuOrdSinRep($\alpha$))\\
\begin{algorithm}[H]
\Orden{O(posicionRelativa(s,a))\footnote{Esta complejidad se cumple por la informaci\'on que brinda la interfaz de SecuOrdSinRep}}
\BlankLine
res$\leftarrow$ esta?(a,s)
\end{algorithm}