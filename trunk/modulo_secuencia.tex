...
\textit{i}insertarOrdenadoDesdeAtras(in a: $\alpha$, inout e: estrSecuOrdSinRep)\\
\begin{algorithm}[H]
\Orden{O(longitud(e) - posicionRelativa(a,e))}
\BlankLine
\textbf{var} n: nodo\\
\BlankLine
n.dato $\leftarrow$ a \tcp*{O(1)}
\BlankLine
\eIf{($\neg$ vacia?(e))}{
	\eIf{(e.prim = e.ult $\vee$ (*e.prim).dato $\opsub{>}{\alpha}$ a $\vee$ (*e.ult).dato $\opsub{<}{\alpha}$ a)}{
		\eIf{((*e.prim).dato $\opsub{>}{\alpha}$ a)}{
			(*e.prim).anterior $\leftarrow$ $\&$n\\
			n.anterior $\leftarrow$ NULL\\
			n.siguiente $\leftarrow$ e.prim\\
			e.prim $\leftarrow$ $\&$n}{
				\If{((*e.prim).dato $\opsub{<}{\alpha}$ a)}{
					(*e.ult).siguiente $\leftarrow$ $\&$n\\
					n.anterior $\leftarrow$ e.ult\\
					n.siguiente $\leftarrow$ NULL\\
					e.ult $\leftarrow$ $\&$n}}}{
	\textbf{var} aux: puntero(nodo)\\
	aux $\leftarrow$ e.ult\\
	\While{((*(*aux).anterior).dato $\opsub{>}{\alpha}$ a)}{aux $\leftarrow$ (*aux).anterior}
