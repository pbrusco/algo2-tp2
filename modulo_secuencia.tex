...
insertar(in a: $\alpha$, inout s: secuOrdSinRep($\alpha$)) \hfill (O(posicionRelativa(s,a)))\\
...
esta?(in a: $\alpha$, in s: secuOrdSinRep($\alpha$))$\longrightarrow$ res: bool\hfill (O(posicionRelativa(as,)))\\
$\{$true$\}$\\
...
avanzar(it) \tcp*{O(1)}} 
\BlankLine
\eIf{(a $\opsub{<}{\alpha}$ actual(it))\tcp*{O(1)}}{
