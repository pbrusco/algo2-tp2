...
\begin{algorithm}[H]
\Orden{O(posicionRelativa(elem,e))}
\BlankLine
\textbf{var} n: nodo\\
\BlankLine
...
\eIf{($\neg$ vacia?(e))\tcp*{O(1)}}{
	\eIf{((*e.prim).dato $\opsub{>}{\alpha}$ elem $\vee$ (*e.ult).dato $\opsub{<}{\alpha}$ elem)\tcp*{O(1)}}{
		\eIf{((*e.prim).dato $\opsub{>}{\alpha}$ elem)\tcp*{O(1)}}{
			...
			n.siguiente $\leftarrow$ e.prim\tcp*{O(1)}
			e.prim $\leftarrow$ $\&$n\tcp*{O(1)}}{
				\If{((*e.prim).dato $\opsub{<}{\alpha}$ elem)\tcp*{O(1)}}{
					(*e.ult).siguiente $\leftarrow$ $\&$n\tcp*{O(1)}
					n.anterior $\leftarrow$ e.ult\tcp*{O(1)}
					n.siguiente $\leftarrow$ NULL\tcp*{O(1)}
					e.ult $\leftarrow$ $\&$n\tcp*{O(1)}}}}{
	\textbf{var} aux: puntero(nodo)\tcp*{O(1)}
	aux $\leftarrow$ e.ult\tcp*{O(1)}
	\While{((*(*aux).anterior).dato $\opsub{>}{\alpha}$ elem)\tcp*{O(posicionRelativa(elem,e))}}{aux $\leftarrow$ (*aux).anterior\tcp*{O(1)}}
		(*(*aux).anterior).siguiente $\leftarrow$ $\&$n\tcp*{O(1)}
		n.anterior $\leftarrow$ (*aux).anterior\tcp*{O(1)}
		n.siguiente $\leftarrow$ aux\tcp*{O(1)}
...
\begin{algorithm}[H]
\Orden{O(a)\footnote{a = longitud(e) - posicionRelativa(elem,e)}}
\BlankLine
\textbf{var} n: nodo\\
\BlankLine
n.dato $\leftarrow$ elem \tcp*{O(1)}
...
\eIf{($\neg$ vacia?(e))\tcp*{O(1)}}{
	...
	\textbf{var} aux: puntero(nodo)\tcp*{O(1)}
	aux $\leftarrow$ e.ult\tcp*{O(1)}
	\While{((*(*aux).anterior).dato $\opsub{>}{\alpha}$ elem)\tcp*{O(a)}}{aux $\leftarrow$ (*aux).anterior\tcp*{O(1)}}
		(*(*aux).anterior).siguiente $\leftarrow$ $\&$n\tcp*{O(1)}
		n.anterior $\leftarrow$ (*aux).anterior\tcp*{O(1)}
		n.siguiente $\leftarrow$ aux\tcp*{O(1)}
		(*aux).anterior $\leftarrow$ $\&$n\tcp*{O(1)}}}{
